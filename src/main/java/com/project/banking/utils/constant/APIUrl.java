package com.project.banking.utils.constant;

public class APIUrl {
    public static final String AUTHENTICATION_API = "/api/v1/auth";
    public static final String BANK_ACCOUNT_API = "/api/v1/bank-accounts";
    public static final String BRANCH_API = "/api/v1/branches";
    public static final String PROFILE_API = "/api/v1/profiles";
    public static final String TRANSACTION_API = "/api/v1/transactions";
    public static final String CARD_API = "/api/v1/cards";

    public static final String PATH_ID = "/{id}";
    public static final String PATH_REGISTER = "/register";
    public static final String PATH_LOGIN = "/login";
    public static final String PATH_ADD_ROLE = "/role";
    public static final String PATH_VERIFY = "/verify-account";
    public static final String PATH_RESET_PASS = "/reset-password";
    public static final String PATH_UNLOCK = "/unlock-account";
    public static final String PATH_CLOSE = "/close";
    public static final String PATH_TRANSFER = "/transfer";
    public static final String PATH_CASH = "/cash-flow";
    public static final String PATH_DEPOSIT = "/deposit";
    public static final String PATH_WITHDRAW = "/withdraw";
    public static final String PATH_SENDER = "/out";
    public static final String PATH_RECEIVER = "/in";
    public static final String PATH_UNBLOCK = "/unblock";
}
